            ...
            for idx, signal in enumerate(hma_signals):
                try:
                    ...
                    if signal['type'] == 'BUY':
                        ...
                        y = band_val - offset
                        ax1.scatter(date, float(y.item()) if hasattr(y, 'item') else float(y), color=color, marker=marker, s=50, label=None)
                    else:
                        band_val = upper_band.loc[date] if safe_in_index(upper_band.index, date) else signal['price']
                        y = band_val + offset
                        ax1.scatter(date, float(y.item()) if hasattr(y, 'item') else float(y), color=color, marker=marker, s=50, label=None)
                    ax1.text(
                        float(mdates.date2num(date)), float(y.item()) if hasattr(y, 'item') else float(y), date.strftime('%Y-%m-%d'),
...
def plot_signals_with_strength(data: pd.DataFrame, ticker: str, save_path: str = None) -> None:
    ...
    try:
        ...
        ax2_price.tick_params(axis='y', labelleft=True, labelright=False, left=True, right=False, colors='blue')
        ax2_price.spines['right'].set_visible(False)
        # 거래량(우측 y축)
        ax2.set_ylabel('Volume', color='gray')
        ax2.yaxis.set_label_position('right')
        ax2.yaxis.tick_right()
        ax2.tick_params(axis='y', labelleft=False, labelright=True, left=False, right=True, colors='gray')
        ax2.spines['left'].set_visible(False)
        # y축 범위 고정 (가격: min/max에 5% 여유, 거래량: 0~최대값에 10% 여유)
        ma5_min = to_float(ma5_price.min())
        ma20_min = to_float(ma20_price.min())
        ma60_min = to_float(ma60_price.min())
        ma120_min = to_float(ma120_price.min())
        ma200_min = to_float(ma200_price.min())
        ma_min = min(ma5_min, ma20_min, ma60_min, ma120_min, ma200_min)
        ma5_max = to_float(ma5_price.max())
        ma20_max = to_float(ma20_price.max())
        ma60_max = to_float(ma60_price.max())
        ma120_max = to_float(ma120_price.max())
        ma200_max = to_float(ma200_price.max())
        ma_max = max(ma5_max, ma20_max, ma60_max, ma120_max, ma200_max)
        price_min = ma_min
        price_max = ma_max
        price_margin = (price_max - price_min) * 0.05
        ax2_price.set_ylim(price_min - price_margin, price_max + price_margin)
        vol_max = to_float(data['Volume'].max())
        ax2.set_ylim(0, vol_max * 1.1)
        # MA5/MA20 골든/데드크로스 시그널 마커 및 범례
        ma_buy_idx = []
        ma_sell_idx = []
        for i in range(1, len(ma5_price)):
            prev_ma5 = ma5_price.iloc[i-1]
            prev_ma20 = ma20_price.iloc[i-1]
            curr_ma5 = ma5_price.iloc[i]
            curr_ma20 = ma20_price.iloc[i]
            # Series → float 변환
            prev_ma5 = prev_ma5.item() if hasattr(prev_ma5, 'item') else float(prev_ma5)
            prev_ma20 = prev_ma20.item() if hasattr(prev_ma20, 'item') else float(prev_ma20)
            curr_ma5 = curr_ma5.item() if hasattr(curr_ma5, 'item') else float(curr_ma5)
            curr_ma20 = curr_ma20.item() if hasattr(curr_ma20, 'item') else float(curr_ma20)
            # NaN 체크
            if any([pd.isna(prev_ma5), pd.isna(prev_ma20), pd.isna(curr_ma5), pd.isna(curr_ma20)]):
                continue
        ...
        # 3~5번 subplot y축 tick/label 복원 (기본값)
        ax3.tick_params(axis='y', labelleft=True, labelright=False, left=True, right=False)
        ax4.tick_params(axis='y', labelleft=True, labelright=False, left=True, right=False)
        ax5.tick_params(axis='y', labelleft=True, labelright=False, left=True, right=False)
        
        # subplot 3,4,5: Y축 위치 조정 (VIX: 우측, MACD/RSI: 우측)
        # VIX 도표(ax3): Y축을 오른쪽으로
        ax3.yaxis.set_label_position('right')
        ax3.yaxis.tick_right()
        ax3.tick_params(axis='y', labelleft=False, labelright=True, left=False, right=True)
