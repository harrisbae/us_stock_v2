...
def get_hma_mantra_md_signals(data: pd.DataFrame) -> list:
    """
    매수 신호 필수: HMA 우상향
    ...
    for i in range(5, len(data)):
        ...
        # HMA5 평균값 계산 (직전 5일)
        hma_5avg = np.mean([hma.iloc[i-j].item() for j in range(1, 6)])
        hma_5avg_prev = np.mean([hma.iloc[i-1-j].item() for j in range(1, 6)])
        # HMA5 상승 조건: 오늘 HMA > 전일 기준 5일 HMA 평균 또는 HMA 변화폭(절대값) < 0.6
        # HMA 기울기 계산 (픽셀 단위)
        # 해상도상의 HMA 값 계산 (픽셀 단위)
        hma_now_pixel = hma_now * 100  # 100은 스케일링 팩터
        hma_prev_pixel = hma_prev * 100
        pixel_slope = (hma_now_pixel - hma_prev_pixel) / (data.index[i] - data.index[i-1]).total_seconds()
        is_hma5_up = (hma_now > hma_5avg_prev) or (abs(pixel_slope) < 0.6)
        # 매수 조건: HMA5 상승(필수)
        if is_hma5_up:
            ...
            if (close < lower) and (macd_now <= macd_signal_now) and (rsi_now >= 40):
                ...
                continue
        # 매도 조건: HMA5 평탄/하락(필수)
        if (hma_now <= hma_5avg_prev) or (abs(hma_now - hma_prev) < 0.15):
            # 1) HMA 하향 돌파 + MACD < Signal
...
def plot_hma_mantra_md_signals(data, ticker, save_path=None):
    """HMA와 만트라 밴드 기반 매매 신호를 시각화합니다."""
    hma = calculate_hma(data['Close'])
    upper_band, lower_band = calculate_mantra_bands(data['Close'])
    signals = get_hma_mantra_md_signals(data)
