...
def plot_hma_mantra_md_signals(data, ticker, save_path=None, flat_threshold=0.6):
    ...
    fig, axes = plt.subplots(8, 1, figsize=(15, 27), sharex=True, gridspec_kw={'height_ratios': [3, 1, 1, 1, 1, 1, 1, 1]})
    ax_main, ax_macd, ax_rsi, ax_volume, ax_vix, ax_tnx, ax_dxy, ax_gold = axes
    ...
    current_upper_bollinger = upper_bollinger.iloc[-1]
    current_lower_bollinger = lower_bollinger.iloc[-1]

    # 현재가 수평선
    ax_main.axhline(y=current_price, color='black', linestyle='-', linewidth=0.8, alpha=0.5, label=f'현재가: {current_price:.2f}')
    # HMA 수평선
    ax_main.axhline(y=current_hma, color='blue', linestyle='-', linewidth=0.8, alpha=0.5, label=f'HMA: {current_hma:.2f}')
    # 만트라 밴드 수평선
    ax_main.axhline(y=current_upper, color='red', linestyle='--', linewidth=0.8, alpha=0.5, label=f'상단밴드: {current_upper:.2f}')
    ax_main.axhline(y=current_lower, color='green', linestyle='--', linewidth=0.8, alpha=0.5, label=f'하단밴드: {current_lower:.2f}')
    # 볼린저 밴드 수평선
    ax_main.axhline(y=current_sma, color='deepskyblue', linestyle='--', linewidth=0.8, alpha=0.5, label=f'SMA20: {current_sma:.2f}')
    ax_main.axhline(y=current_upper_bollinger, color='orange', linestyle='-', linewidth=0.8, alpha=0.5, label=f'볼린저상단: {current_upper_bollinger:.2f}')
    ax_main.axhline(y=current_lower_bollinger, color='orange', linestyle='-', linewidth=0.8, alpha=0.5, label=f'볼린저하단: {current_lower_bollinger:.2f}')
    ...
    # x축(일자) 설정
    for ax in [ax_main, ax_macd, ax_rsi, ax_volume, ax_vix, ax_tnx, ax_dxy, ax_gold]:
    ...
    for signal in signals:
        ...
        if date in data.index:
            ...
            ax_macd.scatter(date, y_macd, color=color, s=30, zorder=5)  # 교차점에 점 추가
            ...
            ax_rsi.scatter(date, y_rsi, color=color, s=30, zorder=5)  # 교차점에 점 추가
            ...
            if date in valid_data.index:
                ...
                ax_volume.scatter(date, y_vol, color=color, s=30, zorder=5)  # 교차점에 점 추가
            ...
            if 'vix' in locals() and date in vix.index:
                ...
                if hasattr(y_vix, 'item'):
                    ...
                    y_vix = float(y_vix)
                ax_vix.scatter(date, y_vix, color=color, s=30, zorder=5)  # 교차점에 점 추가
            ...
            if 'tnx' in locals() and date in tnx.index:
                ...
                if hasattr(y_tnx, 'item'):
                    ...
                    y_tnx = float(y_tnx)
                ax_tnx.scatter(date, y_tnx, color=color, s=30, zorder=5)  # 교차점에 점 추가
            ...
            if 'dxy' in locals() and date in dxy.index:
                ...
                if hasattr(y_dxy, 'item'):
                    ...
                    y_dxy = float(y_dxy)
                ax_dxy.scatter(date, y_dxy, color=color, s=30, zorder=5)  # 교차점에 점 추가
            ...
            if 'gold' in locals() and date in gold.index:
                ...
                if hasattr(y_gold, 'item'):
                    ...
                    y_gold = float(y_gold)
                ax_gold.scatter(date, y_gold, color=color, s=30, zorder=5)  # 교차점에 점 추가
