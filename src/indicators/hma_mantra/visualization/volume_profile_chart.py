"""
Volume Profile이 포함된 메인차트 시각화 모듈
"""
...
def plot_main_chart_with_volume_profile(data: pd.DataFrame, ticker: str = None, save_path: str = None, current_price: float = None):
    """Volume Profile이 포함된 메인차트"""
    # 폰트 설정
    ...
    # 차트 생성 (2x2 레이아웃: 메인차트 + 거래량, Volume Profile)
    fig = plt.figure(figsize=(20, 12))
    gs = GridSpec(2, 2, width_ratios=[4, 1], height_ratios=[3, 1], figure=fig, hspace=0.1, wspace=0.05)
    ...
    # Volume Profile (우측 전체) - 메인차트와 동일한 Y축 범위 사용
    ax_volume_profile = fig.add_subplot(gs[:, 1])
    ...
    # Volume Profile 표시 (우측)
    bin_heights = price_bins[1] - price_bins[0]
    ax_volume_profile.barh(price_bins[:-1], volume_profile, height=bin_heights, alpha=0.6, color='blue', label='Volume Profile')
    
    # 메인차트의 Y축 범위 가져오기
    main_ylim = ax_main.get_ylim()
    main_yticks = ax_main.get_yticks()
    main_yticklabels = ax_main.get_yticklabels()
    
    # Volume Profile의 Y축을 메인차트와 정확히 일치하도록 설정
    ax_volume_profile.set_ylim(main_ylim)
    ax_volume_profile.set_yticks(main_yticks)
    ax_volume_profile.set_yticklabels(main_yticklabels)
    
    # Volume Profile의 Y축 위치를 메인차트와 정확히 일치하도록 조정
    ax_volume_profile.spines['left'].set_position(('data', ax_main.get_xlim()[1]))
    ax_volume_profile.spines['left'].set_visible(False)  # Volume Profile의 왼쪽 Y축 숨기기
    ax_volume_profile.yaxis.set_ticks_position('right')  # Y축 눈금을 오른쪽으로 이동
    ...
    # Value Area 표시
    ax_volume_profile.axhspan(value_area_min, value_area_max, alpha=0.2, color='green', label=f'Value Area: {value_area_min:.2f}-{value_area_max:.2f}')
    
    # Volume Profile 설정
    ax_volume_profile.set_xlabel('Volume')
    ax_volume_profile.set_title('Volume Profile')
    ax_volume_profile.legend(fontsize=8)
    ax_volume_profile.grid(True, alpha=0.3)

    # 메인차트 설정
    ax_main.set_title(f'{ticker} - Main Chart with Volume Profile ({start_date.strftime("%Y-%m-%d")} ~ {end_date.strftime("%Y-%m-%d")})')
    ...
    # 레이아웃 조정 (tight_layout 대신 수동 조정)
    plt.subplots_adjust(left=0.08, right=0.95, top=0.95, bottom=0.08, hspace=0.1, wspace=0.05)
