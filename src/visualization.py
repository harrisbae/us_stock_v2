...
class StockVisualizer:
    ...
    def _plot_candlestick_with_bb(self):
        ...
        if all(col in self.data.columns for col in ['BB_Upper', 'BB_Middle', 'BB_Lower']):
            # NaN 값 제거
            valid_mask = (~self.data['BB_Upper'].isna()) & (~self.data['BB_Middle'].isna()) & (~self.data['BB_Lower'].isna())
            
            ax.plot(self.data.index[valid_mask], self.data.loc[valid_mask, 'BB_Upper'], 'k--', alpha=0.3, label='BB 상단')
            ax.plot(self.data.index[valid_mask], self.data.loc[valid_mask, 'BB_Middle'], 'k-', alpha=0.3, label='BB 중간')
            ax.plot(self.data.index[valid_mask], self.data.loc[valid_mask, 'BB_Lower'], 'k--', alpha=0.3, label='BB 하단')
            ax.fill_between(self.data.index[valid_mask], 
                          self.data.loc[valid_mask, 'BB_Upper'], 
                          self.data.loc[valid_mask, 'BB_Lower'], 
...
def plot_hma_mantra_bands(df, symbol, period=20, multiplier=2.0):
    """
    HMA와 만트라 밴드를 시각화
    """
    ...
    # 가격 차트
    plt.subplot(2, 1, 1)
    plt.plot(df.index, df['Close'], label='종가', color='black', alpha=0.7)
    
    # HMA
    hma = calculate_hma(df['Close'], period)
    plt.plot(df.index, hma, label=f'HMA({period})', color='blue', linewidth=2)
    
    # 만트라 밴드
    upper, ma, lower = calculate_mantra_bands(df['Close'], period, multiplier)
    plt.plot(df.index, upper, label=f'만트라 상단({period})', color='red', linestyle='--')
    plt.plot(df.index, ma, label=f'만트라 중간({period})', color='green', linestyle='--')
    plt.plot(df.index, lower, label=f'만트라 하단({period})', color='red', linestyle='--')
    
    plt.title(f'{symbol} - HMA & 만트라 밴드')
    plt.legend()
    plt.grid(True)
    
    # 거래량
    plt.subplot(2, 1, 2)
    plt.bar(df.index, df['Volume'], label='거래량', color='gray', alpha=0.5)
    plt.title('거래량')
    plt.grid(True)
    
    plt.tight_layout()
    plt.savefig(f'output/{symbol}_hma_mantra.png')
    plt.close() 